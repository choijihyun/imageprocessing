...
void ideal() {
	for (int i = 0; i < X_MAX; i++) {
		for (int j = 0; j < X_MAX; j++) {
			H[i][j] = (double)(1.0-exp(-1 * ((double)(X_MAX / 2.0 - i)*(double)(X_MAX / 2.0 - i) + (double)(X_MAX / 2.0 - j)*(double)(X_MAX / 2.0 - j)) / (2.0 * r*r)));
		}
	}
}
...
int main() {
	...
	printf("HPF in frequency domain - Gaussian\n");
	ideal();
	...
	for (u = 0; u < X_MAX; u++) {
		for (v = 0; v < X_MAX; v++) {
			...
			//printf("sum.real = %f\tH[%d][%d] = %f", sum.real, u, v, H[u][v]);
	...
	out = fopen("lena_HPF_GAUSSIAN_FREQ.img", "wb");
	fwrite(out_data, sizeof(unsigned char), X_MAX*Y_MAX, out);
	fclose(out);
	printf("success to write HPF_DFT file\n");
	...
	out = fopen("lena_HPF_GAUSSIAN_SPATIAL.img", "wb");
	fwrite(out_data2, sizeof(unsigned char), X_MAX*Y_MAX, out);
	fclose(out);
	printf("success to write Inverse lena_HPF_GAUSSIAN_SPATIAL file\n");
}