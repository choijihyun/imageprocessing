...
int main() {
	...
	printf("HPF in frequency domain\n");
	ideal();
	...
	for (u = 0; u < X_MAX; u++) {
		for (v = 0; v < X_MAX; v++) {
			...
			sum.real = sum.real / (X_MAX*X_MAX)*H[u][v];
			sum.image = sum.image / (X_MAX*X_MAX)*H[u][v];
			...
			IDEAL[u][v].real = sum.real;
			IDEAL[u][v].image = sum.image;

		}
	}

	out = fopen("lena_HPF_IDEAL_FREQ.img", "wb");
	...
	out = fopen("lena_HPF_IDEAL_SPATIAL.img", "wb");
	fwrite(out_data2, sizeof(unsigned char), X_MAX*Y_MAX, out);
	fclose(out);
	printf("success to write Inverse HPF_IDEAL_SPATIAL file\n");
}
